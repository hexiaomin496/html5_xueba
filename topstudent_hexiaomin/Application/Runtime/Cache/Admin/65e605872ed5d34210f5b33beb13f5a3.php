    ...
    <div class="row content">
        ...
        <div class="col-md-10 col-xs-10 right">
			...
			<div class="buttongroup">
            	<form class="navbar-form" action="/topstudent/index.php/Admin/user/searchUser" >
            ...
            <div class="tablegroup">
            	<table class="table">
                 <form action="/topstudent/index.php/Admin/user/closureAll" method="get">
                	<tr>
                    	<th>&nbsp;</th>
                        <th>用户名</th>
                        <th>积分</th>
                ...
                <?php if(is_array($user)): $i = 0; $__LIST__ = $user;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$user): $mod = ($i % 2 );++$i;?><tr>
                    	<td><input type="checkbox" name="id[]" value="<?php echo ($user["user_id"]); ?>"></td>
                        <td><?php echo ($user["user_username"]); ?></td>
                        <td><?php echo ($user["user_score"]); ?></td>
                        <td><?php echo ($user["user_status"]); ?></td>
                        <td><a href="<?php echo U('Admin/user/detail');?>/id/<?php echo ($user["user_id"]); ?>">查看</a></td>
                        <td><a href="<?php echo U('Admin/user/closure');?>/id/<?php echo ($user["user_id"]); ?>"><?php echo ($user["user_action"]); ?></a></td>
                    </tr><?php endforeach; endif; else: echo "" ;endif; ?>              
                    <tr class="table-bottom">
                    	<td><input type="checkbox"  value="" onclick="if(this.checked==true) { checkAll('id[]'); } else { clearAll('id[]'); }"></td>
                        <td style="text-align:left">全选<button class="button4" type="submit">批量封禁</button></td>
